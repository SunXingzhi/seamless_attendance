...
public class DeviceBitmapService {
    ...
    public void processDevicePersonnelBitmap(String originDeviceName, String personnelName) {
        try {
            logger.info("开始处理设备人员字模: originDeviceName={}, personnelName={}", originDeviceName, personnelName);
            
            // 1. 计算 INDEX
            int index = calculatePersonnelIndex(originDeviceName, personnelName);
            if (index == -1) {
                logger.error("计算 INDEX 失败: originDeviceName={}, personnelName={}", originDeviceName, personnelName);
                return;
            }
