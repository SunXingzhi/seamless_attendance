			...
			<div class="form-section">
				...
				<div class="personnel-selection">
					...
					<div v-else class="personnel-list">
						<div v-for="person in personnel" :key="person.userId"
							class="personnel-item">
							<label :for="`person-${person.userId}`"
								class="personnel-checkbox">
								<input type="checkbox" :id="`person-${person.userId}`"
									:value="person.userNumber"
									v-model="selectedPersonnel" />
								<div class="personnel-info">
									<strong>{{ person.userName }}</strong>
									<span class="personnel-number">编号: {{
										person.userNumber }}</span>
									<span v-if="person.attendanceStatus"
										class="personnel-status">状态: {{
											person.attendanceStatus }}</span>
								</div>
							</label>
						</div>
...
const availablePersonnelNames = computed(() => {
	return personnel.value.map(person => person.userName).join(', ')
})
...
const submitForm = async () => {
	...
	try {
		...
		const submitData = {
			...
			studio_code: studioForm.studio_code,
			description: studioForm.description,
			personnels: selectedPersonnelNumbers.join(','),
		...
		await studioStore.addStudio(submitData)
		alert('工作室创建成功')
		router.push('/studios')
	} catch (error) {
		alert('创建工作室失败: ' + error.message)
	} finally {
